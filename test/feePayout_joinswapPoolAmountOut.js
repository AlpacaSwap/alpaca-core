...
contract('swap fee tests - joinswapPoolAmountOut', async (accounts) => {
    ...
    describe('Test joinswapPoolAmountOut with fee payout', () => {
        it('Test Join should give right share of pool fees', async () => {
            ...
            //     uint maxAmountIn
            // )
            result = await crpPool.joinswapPoolAmountOut(
                tokenAddresses[0],
                toWei('5.9096703681414132'), 
                MAX,
                { from: user1 },
            );

            tokenAmountIn = result.logs[0]['args']['tokenAmountIn'];
            console.log(`Token Amount Out: ${fromWei(tokenAmountIn)}`)  // should be 10  
            ...
            errorCheck(fromWei(tokenAmountIn), 10);
